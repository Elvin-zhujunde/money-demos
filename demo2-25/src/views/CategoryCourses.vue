...
const filteredCourses = computed(() => {
  let courses = categoryCourses.value[selectedCategory.value] || []
...
const fetchData = async () => {
  try {
    ...
    categoryCourses.value = data.categoryCourses
  } catch (error) {
...
onMounted(() => {
  fetchData().then(() => {
    if (!selectedCategory.value || !categoryCourses.value[selectedCategory.value]) {
...
watch(
  () => route.params.id,
  (newId) => {
    if (newId && categoryCourses.value[newId]) {
