...
const course = computed(() => {
  ...
  // 先从热门课程中查找
  let foundCourse = courseData.value.hotCourses.find(c => c.id === courseId)
  
  // 如果热门课程中没找到，则在分类课程中查找
  if (!foundCourse) {
    for (const categoryId in courseData.value.categoryCourses) {
      const categoryCourses = courseData.value.categoryCourses[categoryId]
